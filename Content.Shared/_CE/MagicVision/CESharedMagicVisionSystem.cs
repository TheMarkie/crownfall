...
public abstract class CESharedMagicVisionSystem : EntitySystem
{
    ...
    public readonly EntProtoId MagicTraceProto = "CEMagicVisionMarker";

    public override void Initialize()
    {
        base.Initialize();

        SubscribeLocalEvent<CEMagicVisionMarkerComponent, ExaminedEvent>(OnExamined);
        SubscribeLocalEvent<CEMagicVisionStatusEffectComponent, StatusEffectRelayedEvent<MobStateChangedEvent>>(OnMobStateChange);
    }

    private void OnMobStateChange(Entity<CEMagicVisionStatusEffectComponent> ent, ref StatusEffectRelayedEvent<MobStateChangedEvent> args)
    {
    ...
    protected virtual void OnExamined(Entity<CEMagicVisionMarkerComponent> ent, ref ExaminedEvent args)
    {
        ...
        sb.Append($"{Loc.GetString("ce-magic-vision-timed-past")} {timePassed.Minutes}:{timePassed.Seconds:D2}\n");
        ...
        {
            sb.Append($"{Loc.GetString("ce-magic-vision-aura")} {imprint}");
            args.AddMarkup(sb.ToString());
            return;
        }
        ...
        sb.Append($"{Loc.GetString("ce-magic-vision-aura")} {obscuredImprint}");

        args.AddMarkup(sb.ToString());
    }

    public string? GetAuraImprint(Entity<CEAuraImprintComponent?> ent)
    {
        if (!Resolve(ent, ref ent.Comp))
            return null;

        if (_statusEffects.TryEffectsWithComp<CEHideMagicAuraStatusEffectComponent>(ent, out var hideComps))
        {
        ...
        EntityCoordinates? target = null)
    {
        ...
        var markerComp = EnsureComp<CEMagicVisionMarkerComponent>(ent);
